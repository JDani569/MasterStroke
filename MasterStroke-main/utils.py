...
def allowed_file(filename):
    ...
    return "." in filename and filename.rsplit(".", 1)[
        1
    ].lower() in Config.VIDEO_EXTENSIONS + ["jpg", "jpeg", "png"]
...
def calculate_progress(user_id, course_id):
    ...
    for video in videos:
        progress = Progress.query.filter_by(
            user_id=user_id, video_id=video.id, completed=True
        ).first()
...
def get_user_stats(user_id):
    ...
    videos_watched = Progress.query.filter_by(user_id=user_id, completed=True).count()

    # Get total quizzes attempted
    quizzes_attempted = Attempt.query.filter_by(user_id=user_id).count()
...
def get_trending_courses(limit=5):
    ...
    trending = (
        db.session.query(Course, func.count(Progress.id).label("activity_count"))
