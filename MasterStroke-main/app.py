...
def course(course_id):
    ...
    return render_template(
        "course.html", course=course, videos=videos, video_progress=progress
    )
...
def video(video_id):
    ...
    return render_template(
        "video.html", video=video, quizzes=quizzes, comments=comments, progress=progress
    )
...
def profile():
    ...
    for video in videos:
        ...
        if next(
            (p for p in user_progress if p.video_id == video.id and p.completed), None
        ):
...
def admin_dashboard():
    ...
    most_watched = (
        db.session.query(Video, db.func.count(Progress.id).label("view_count"))
        .join(Progress)
        .group_by(Video.id)
